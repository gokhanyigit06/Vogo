...
async function getProjects() {
    try {
        const q = query(collection(db, "projects"), where("isLabProject", "==", false))
        const snapshot = await getDocs(q)

        const projects = await Promise.all(snapshot.docs.map(async (docSnap) => {
            const data = docSnap.data()
            let clientData = null
            if (data.clientId) {
                const clientDoc = await getDoc(doc(db, "clients", data.clientId))
                if (clientDoc.exists()) {
                    clientData = { company: clientDoc.data().company }
                }
            }
