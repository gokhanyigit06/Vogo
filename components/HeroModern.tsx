...
function FloatingBadge({ children, className, delay = 0 }: { children: React.ReactNode, className?: string, delay?: number }) {
    return (
        <motion.div
            ...
            className={`absolute backdrop-blur-md bg-white/5 border border-white/10 rounded-2xl p-4 shadow-2xl ${className}`}
        >
            {children}
        </motion.div>
    )
...
function Spotlight({ className = "" }: { className?: string }) {
    return (
        <div className={`relative w-full h-full overflow-hidden ${className}`}>
            <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[1000px] h-[500px] bg-emerald-500/20 rounded-[100%] blur-[100px] opacity-50 animate-pulse" />
...
export default function HeroModern() {
    ...
    return (
        <section ref={containerRef} className="relative min-h-[110vh] bg-slate-950 flex flex-col items-center pt-32 overflow-hidden">
            ...
            <div className="container mx-auto px-4 relative z-10 text-center">
                ...
                >
                    ...
                    <span className="text-sm text-slate-300 font-medium">Vogo Agency 2.0 Yayında</span>
                </motion.div>
                ...
                <motion.h1
                    ...
                    className="text-6xl md:text-8xl lg:text-9xl font-bold tracking-tighter text-white mb-8 relative"
                >
                    <span className="block">Digital</span>
                    <span className="block text-transparent bg-clip-text bg-gradient-to-r from-emerald-400 via-cyan-400 to-purple-400 pb-4">
                        Revolution
                    </span>
                </motion.h1>
                ...
                >
                    Markanızı sıradanlıktan çıkarıp <span className="text-white font-semibold">dijital geleceğe</span> taşıyoruz.
                ...
                >
                    <button
                        ...
                        className="group relative px-8 py-4 bg-white text-slate-950 rounded-full font-bold text-lg hover:bg-emerald-400 transition-all duration-300 flex items-center gap-2"
                    >
                ...
                <motion.div style={{ y: y2 }} className="absolute bottom-1/4 left-[15%] hidden lg:block">
                    <FloatingBadge delay={1.5} className="rotate-[12deg]">
                        <div className="flex items-center gap-3">
                            ...
                            <span className="text-white/80 font-mono text-sm">Design_System.fig</span>
                        </div>
                    </FloatingBadge>
                </motion.div>
                ...
                <motion.div style={{ y: y1 }} className="absolute bottom-1/3 right-[15%] hidden lg:block">
                    <FloatingBadge delay={2} className="rotate-[-12deg]">
                        <div className="flex items-center gap-3">
                            ...
                            <span className="text-white/80 font-medium text-sm">Conversion Rate</span>
                        </div>
                    </FloatingBadge>
                </motion.div>
            </div>
